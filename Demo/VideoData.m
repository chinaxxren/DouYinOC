...
+ (NSArray<VideoData *> *)testItems {
    NSMutableArray<VideoData *> *m = [[NSMutableArray alloc] initWithCapacity:20];
    for (int i = 0; i < 20; ++i) {
        [m addObject:[VideoData testItem]];
    }
    return m;
}

+ (NSArray<VideoData *> *)testItemsWithCount:(NSInteger)count {
    NSMutableArray<VideoData *> *m = [[NSMutableArray alloc] initWithCapacity:count];
...
+ (instancetype)testItem {
    ...
    item.title = mediaTitles[arc4random() % mediaTitles.count];
    item.cover = covers[arc4random() % covers.count];
    item.avatar = avatars[arc4random() % avatars.count];
    item.nickname = usernames[arc4random() % usernames.count];
    
    NSURL *URL;
    static NSInteger __index = 0;
    if (__index < urLs.count) {
        URL = [NSURL URLWithString:urLs[__index]];
    } else {
        URL = [NSURL URLWithString:urLs[arc4random() % urLs.count]];
    }
     
    NSURLComponents *comp = [NSURLComponents componentsWithURL:URL resolvingAgainstBaseURL:NO];
    static NSInteger idx;
    [comp setQuery:[NSString stringWithFormat:@"idx=%ld", ++idx]];
    item.videoUrl = comp.URL.absoluteString;
